...
for cid, meta in CONFIG["characters"].items():
    p = BASE / "static" / "characters" / meta["file"]
    characters[cid] = {
        **meta,
        "src": data_uri(p, "image/png"),
    }

backgrounds = {}
for key, filename in CONFIG["backgrounds"].items():
    p = BASE / "static" / "backgrounds" / filename
    backgrounds[key] = data_uri(p, "image/jpeg")

music = {}
for key, filename in CONFIG["music"].items():
    p = BASE / "static" / "audio" / filename
    music[key] = data_uri(p, "audio/mpeg")
